...
export class UsersOrdersComponent implements OnInit {
  ...
  displayedColumns: string[] = [' ', 'name', 'subscriber', 'status', 'date'];
  ...
  ngOnInit(): void {
    this.id = this.route.snapshot.parent?.params['id'].slice(5);
    this.role = this.route.snapshot.parent?.params['id'].slice(0, 5);
    this.orders$ = this.store.select(getOrders).pipe(
      map((orders) => {
        return orders.filter((order) => order.subscribersId === this.id)
      }),
      map((orders) => {
